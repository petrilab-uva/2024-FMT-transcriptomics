...
# Import sample data
metadata <- read.csv("../data/day2_RNAseq_metadata.csv")
...
metadata$Treatment[metadata$Treatment == "FMT Donor"] <- "FMT_Donor"
metadata$Treatment[metadata$Treatment == "FMT Recipient"] <- "FMT_Recipient"
metadata$Treatment[metadata$Treatment == "Vehicle Control"] <- "Vehicle_Control"

metadata$Treatment <- factor(metadata$Treatment, 
                             levels = c("FMT_Donor", "Vehicle_Control",
                                        "FMT_Recipient"))
...
ggplot(data = plotpca, aes(x = PC1, y = PC2, 
  ...
  geom_point(aes(colour = metadata$Treatment), size = 3) +
...
# Save PCA plot
ggsave("../results/figures/PCA plot.png", 
...
rownames(sampleDistMatrix) <- paste(rld$Name, " (", rld$Treatment, ")")
colnames(sampleDistMatrix) <- paste(rld$Name, " (", rld$Treatment, ")")
colors <- colorRampPalette( rev(brewer.pal(9, "Blues")) )(255)

# Visualize heatmap of within-sample variation.
png("../results/figures/heatmap_sample distances.png", 
...
# REMAKE DESEQ OBJECT WITH ONLY ABX AND FMT SAMPLES ----------------------------

# I'm primarily looking at differences between the antibiotic treated and FMT 
# treated samples. I'll make a new DESeq2 object containing only these
# treatment groups and use this for PCA analysis.

# Filter out donor samples
dds_nodonor <- dds[dds$Treatment == "Vehicle_Control" | 
                     dds$Treatment == "FMT_Recipient"]

donor <- c("G3_27", "G3_28", "G3_29", "G3_30")
dds_nodonor <- dds_nodonor[,!colnames(dds_nodonor) %in% donor]

# Re-level Treatment variable
dds_nodonor$Treatment <- factor(dds_nodonor$Treatment, 
                                levels = c("Vehicle_Control", "FMT_Recipient"))

#Re-make DESeq object
dds_nodonor <- DESeq(dds_nodonor)

# Log transform and organize count data for PCA
rld_nodonor <- rlog(dds_nodonor, blind = TRUE)
plotpca_nodonor <- plotPCA(rld_nodonor, intgroup = c("Treatment"), returnData = TRUE)
percentVar <- round(100 * attr(plotpca_nodonor, "percentVar"))


# Organize Data for PCA
plotpca_nodonor_plot <- plotpca_nodonor
plotpca_nodonor_plot$Group[plotpca_nodonor_plot$Treatment == "Vehicle_Control"] <- "PBS Control"
plotpca_nodonor_plot$Group[plotpca_nodonor_plot$Treatment == "FMT_Recipient"] <- "FMT Recipient"
plotpca_nodonor_plot$Group <- factor(plotpca_nodonor_plot$Group, 
                                     levels = c("PBS Control", "FMT Recipient"))

# Plot PCA
ggplot(data = plotpca_nodonor_plot, aes(x = PC1, y = PC2, 
                                   #color = metadata$Treatment,
                                   #label = metadata$Name
)) + 
...
# Save PCA graph.
ggsave("../results/figures/PCA plot_VEHICLEvsFMT.png", 
       dpi = 300)
...
# Get names of possible comparisons
resultsNames(dds_object)

# Generate results files for each comparison.
res_FMTvsVEHICLE <- results(dds_object, contrast = c("Treatment", 
                                                     "FMT_Recipient", 
                                                     "Vehicle_Control"))

res_VEHICLEvsDONOR <- results(dds_object, contrast = c("Treatment", 
                                                       "Vehicle_Control", 
                                                       "FMT_Donor"))

res_FMTvsDONOR <- results(dds_object, contrast = c("Treatment", 
                                                   "FMT_Recipient",
                                                   "FMT_Donor"))
...
#png("../results/figures/dispersion estimates.png", 
#    width = 750, height = 750)
...
#png("../results/figures/MA_FMT_vs_control.png", 
#    width = 750, height = 750)
DESeq2::plotMA(res_FMTvsVEHICLE)
#dev.off()

#png("../results/figures/MA_control_vs_donor.png", 
#    width = 750, height = 750)
DESeq2::plotMA(res_VEHICLEvsDONOR)
#dev.off()

#png("../results/figures/MA_FMT_vs_donor.png", 
#    width = 750, height = 750)
DESeq2::plotMA(res_FMTvsDONOR)
#dev.off()



# Save only results files for downstream analysis
rm(list = setdiff(ls(), c("res_FMTvsVEHICLE", "res_VEHICLEvsDONOR", 
                          "res_FMTvsDONOR", "rld", "dds_object", "metadata")))

save.image(file = "../results/DESeq2 results.RData")
