...
# This script will perform QC and map reads from FASTQs from the Day 7 mouse FMT 
...
# into a "day7_RNAseq_fastqs" directory within the "data" directory. This script 
...
#"chmod +x ./01-fastq_processing_and_readMapping_day7.sh" 
...
for i in ./data/day7_RNAseq_fastqs/*fq.gz
do
...
multiqc ./results \
-d -n report_day7_raw_reads.html \
-o ./results
...
for i in $(basename ./data/day7_RNAseq_fastqs/*.fq.gz | awk '{print substr( $0, 0, 2)}' | uniq)
do 
	bbduk.sh \
	in1=./data/day7_RNAseq_fastqs/"$i"_1.fq.gz \
	in2=./data/day7_RNAseq_fastqs/"$i"_2.fq.gz \
	out1=./data/day7_RNAseq_fastqs/trimmed_reads/"$i"_1_trimmed.fq.gz \
	out2=./data/day7_RNAseq_fastqs/trimmed_reads/"$i"_2_trimmed.fq.gz \
...
for i in ./data/day7_RNAseq_fastqs/trimmed_reads/*fq.gz
do
...
for i in $(basename ./data/day7_RNAseq_fastqs/trimmed_reads/*.fq.gz | awk '{print substr( $0, 0, 2)}' | uniq)
do
	kallisto quant -i ./data/Mus_musculus.GRCm39.cdna.all.index \
	-o ./results/kallisto/"$i" -t 16 \
	./data/day7_RNAseq_fastqs/trimmed_reads/"$i"_1_trimmed.fq.gz \
	./data/day7_RNAseq_fastqs/trimmed_reads/"$i"_2_trimmed.fq.gz \
...
# Summarize post-processed FASTQs and read mapping using MultiQC
multiqc ./results \
--ignore ./results/fastqc/raw_reads \
-d -n report_day7_trimmed_reads_and_mapping.html \
-o ./results
